...
class Database {
  ...
  loadGraphPoints (graphDuration, callback) {
    ...
    this.getRecentPings(startTime, endTime, pingData => {
      ...
      let lastGraphTimestamp = startTime
      ...
      for (const row of pingData) {
        ...
        if (row.timestamp - lastGraphTimestamp >= GRAPH_UPDATE_TIME_GAP) {
          graphTimestamps.push(row.timestamp)
          lastGraphTimestamp = row.timestamp
        }
      }
      ...
      for (const serverRegistration of this._app.serverRegistrations) {
        ...
        const graphData = graphTimestamps.map(timestamp => {
          return playerCounts.has(timestamp) ? playerCounts.get(timestamp) : null
        })

        serverRegistration.loadGraphPoints(startTime, graphTimestamps, graphData)
      }

      this._app.timeTracker.loadGraphPoints(startTime, graphTimestamps)
  ...
  getRecentPings (startTime, endTime, callback) {
    this._sql.all('SELECT * FROM pings WHERE timestamp >= ? AND timestamp <= ? ORDER BY timestamp ASC, rowid ASC', [
      startTime,
      endTime
    ], (err, data) => {
      if (err) {
