  ...
  pingAll = () => {
    ...
    this.startPingTasks(results => {
      ...
      for (const serverRegistration of this._app.serverRegistrations) {
        ...
        updates[serverRegistration.data.name] = update
      }

      const changedColors = this._app.colorAssigner.assignColors(this._app.serverRegistrations)

      Object.entries(changedColors).forEach(([serverId, color]) => {
        const serverRegistration = this._app.serverRegistrations[serverId]

        if (!updates[serverRegistration.data.name]) {
          updates[serverRegistration.data.name] = {}
        }

        updates[serverRegistration.data.name].color = color
      })

      // Send an object since updates uses server names as keys
