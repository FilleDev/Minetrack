...
class Server {
  ...
  handleStatsRequest = (req, res) => {
    ...
    const serversWithPings = this._app.serverRegistrations.filter(serverRegistration => serverRegistration.hasSuccessfulPing())

    for (const serverRegistration of serversWithPings) {
      const serverData = {
