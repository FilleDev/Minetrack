...
class App {
  ...
  handleClientConnection = (client) => {
    if (config.logToDatabase) {
      client.on('message', (message) => {
        if (message.toString() === 'requestHistoryGraph') {
          // Filter servers to only include those with successful pings
          const serversWithPings = this.serverRegistrations.filter(serverRegistration => serverRegistration.hasSuccessfulPing())

          // Send historical graphData built from serverRegistrations with successful pings
          const graphData = serversWithPings.map(serverRegistration => serverRegistration.graphData)
    ...
    // Filter servers to only show those with successful pings
    const serversWithPings = this.serverRegistrations.filter(serverRegistration => serverRegistration.hasSuccessfulPing())
    ...
    const initMessage = {
      config: (() => {
        ...
        return {
          ...
          servers: serversWithPings.map(serverRegistration => serverRegistration.getPublicData()),
          minecraftVersions: minecraftVersionNames,
          isGraphVisible: config.logToDatabase
        }
      })(),
      timestampPoints: this.timeTracker.getServerGraphPoints(),
      servers: serversWithPings.map(serverRegistration => serverRegistration.getPingHistory())
    }
