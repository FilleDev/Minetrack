...
const { loadMinecraftVersions } = require('./lib/minecraft-versions')
...
async function main () {
  ...
  const app = new App(minecraftVersions)
...
main().catch((err) => {
  logger.log('error', 'Unable to load Minecraft version data: %s', err.message)
  process.exitCode = 1
})
